# --------------------------------------------------------
# Based on DentexSegAndDet code bases
# https://github.com/xyzlancehe/DentexSegAndDet
# --------------------------------------------------------'
import os
import json
import random
...
def process_coco_disease():
    ...
    dataset_json = load_json(
        "dentex_dataset/origin/quadrant-enumeration-disease/train_quadrant_enumeration_disease.json"
    )
    ...
    for image in dataset_json["images"]:
        ...
        if image["id"] in train_ids:
            train_json["images"].append(image)
            shutil.copy(
                f"dentex_dataset/origin/quadrant-enumeration-disease/xrays/{image_filename}",
