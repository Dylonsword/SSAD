...
class SSATTrainer(SimpleTrainer):
    def __init__(
        self,
        model,
        auximodel,
        ...
        """
        auximodel.train()
        self.auximodel = auximodel
        self.auxi_optimizer = auxi_optimizer
        # to access the data loader iterator, call `self._data_loader_iter`
        self._data_loader_iter_obj = None
        self.loss_scale = loss_scale
        self.ssat_method = ssat_method
    ...
    def run_step(self):
        ...
        """
        assert self.model.training, "[SimpleTrainer] model was changed to eval mode!"
        assert self.auximodel.training, "[SimpleTrainer] model was changed to eval mode!"
        start = time.perf_counter()
        """
        If you want to do something with the data, you can wrap the dataloader.
        """
        ...
        """
        loss_dict = self.model(data)
        if self.task_m == "diffdet":
            images = torch.stack([self.model.normalizer(d["image"].to(self.model.device)) for d in data])
        else:
            if isinstance(self.model, DistributedDataParallel):
                images = self.model.module.preprocess_image(data)
            else:
                images = self.model.preprocess_image(data)

        if data[0].get("mask", None) is not None:
            masks = torch.stack([torch.from_numpy(d["mask"]).to(self.model.device) for d in data])
        else:
            masks = None

        # needn't transfer to nested_tensor
        if self.task_m == "diffdet":
            encoder = self.model.backbone.bottom_up # TODO: Unwrapper in encode
            self.model.backbone.bottom_up.pixel_mean = self.model.pixel_mean
            self.model.backbone.bottom_up.pixel_std = self.model.pixel_std
        else:
            encoder = self.model
        auxi_loss = self.auximodel(imgs=images, masks=masks, encoder=encoder, task_m=self.task_m, mask_ratio=self.mask_ratio)
        if isinstance(loss_dict, torch.Tensor):
            losses = loss_dict
            loss_dict = {"total_loss": (1-self.loss_scale) * loss_dict + self.loss_scale * auxi_loss}
        else:
            losses = sum(loss_dict.values())
            loss_dict["loss_recon"] = auxi_loss
            losses = (1-self.loss_scale) * losses + self.loss_scale * auxi_loss
