...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, color_assign=None):
        """
    ...
    def draw_dataset_dict(self, dic):
        ...
        if annos:
            ...
            label_color = [np.array([1.0, 0.0, 0.0]), np.array([0.1960, 1.0, 0.0]), 
                        np.array([0.96, 0.647, 0.4313]), np.array([1.0, 1.0, 0.043])]
            colors = [label_color[x] for x in category_ids]
            if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
                colors = [
    ...
    def draw_rotated_box_with_label(
        ...
        linewidth = self._default_font_size / (
            ...
            4 if area < _SMALL_OBJECT_AREA_THRESH * self.output.scale else 2
        )
