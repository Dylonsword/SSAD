...
class ClipExtractor(nn.Module):
    def __init__(self, args, use_RGB=True):
        super().__init__()
        model = clip.load(getattr(args, "clip_model_name", "ViT-B/32"))[0]
                ...
                T.RandomApply(
                    [
                        T.RandomAffine(
                            degrees=15,
                            translate=(0.1, 0.1),
                            fill=getattr(args, "clip_affine_transform_fill", 1),
        ...
        self.n_aug = getattr(args, "n_aug", 2)
