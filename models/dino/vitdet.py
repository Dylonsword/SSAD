...
from detectron2.layers import ShapeSpec, CNNBlockBase, Conv2d, get_norm
...
class SimpleFeaturePyramid(Backbone):
    ...
    def forward(self, x_tensor: NestedTensor):
        """
        Args:
        ...
        outs_dict = {}
        for idx, out_i in enumerate(results):
            if idx in self.return_interm_indices:
                m = x_tensor.mask
...
size2config = {
    "B": {
        ...
        "use_rel_pos": True,
        "window_size": 14,
        "window_block_indexes": (0,1,3,4,6,7,9,10)    # 2,5,8,11 for gobal attention
    },
    "L": {
        "embed_dim": 1024,
        "depth": 24,
...
def build_vit_backbone(cfg):
    ...
    model.num_patches = (img_size // 16) ** 2
    return model

def build_vitdet_backbone(cfg):
    """
    """
    net = build_vit_backbone(cfg)
    in_features = net._out_features[0]
